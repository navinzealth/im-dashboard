...
const headCells = [
  { id: 'cat_name',  numeric: true, disablePadding: false, label: 'Category Name', },
  { id: 'cat_img', numeric: true, disablePadding: false, label: 'Question', },
  { id: 'cat_published',  numeric: true, disablePadding: false, label: 'Status', },
  { id: 'cat_description', numeric: true, disablePadding: false, label: 'Publish Date', },
  { id: 'cat_description', numeric: true, disablePadding: false, label: 'Action', },
];
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" >  Quiz Categories </Typography>
      )}
    </Toolbar>
  );
}
...
export default function QuizTable(props ) {
 ...
 // const visibleRows =  useMemo( () =>  rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage,),  [ page, rowsPerPage], );
  ...
  const changeCategoryStatus = (event) => { 
   // setChecked(!checked)
    console.log(event)
    let data = JSON.stringify({ "userId": `${userId}`, "quizId":  event._id, 'quiz': event.quiz, "opt1":event.opt1, "opt2":event.opt2, 'opt3':event.opt3, 'opt4':event.opt4,'ans':event.ans, "status": event.status=='1' ? '0' : '1'});
    let config = {method: 'post', maxBodyLength: Infinity, url: `${apiRoute}/editquiz`, headers: {  'Content-Type': 'application/json' }, data : data}; 
      axios.request(config)
      .then(fetchAllquizAPI) 
    //  .then((response) => { 
...
const fetchAllquizAPI=useCallback(()=>{
  ...
  axios.request(config)
  .then((response) => { getQuizlist(response.data.Data);  })
              ...
              {visibleRows?.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    tabIndex={-1} key={row.quizId} selected={isItemSelected} sx={{ cursor: 'pointer' }} >
                    ...
                    <TableCell align="right">{row.quizCatName}</TableCell>
                    ...
                    inputProps={{ 'aria-label': 'controlled' }} onChange={()=>changeCategoryStatus(row)}/></TableCell>
                    ...
                    <TableCell align="right"><Link href={`/Quiz/QuizListing/QuizEdit/${row._id}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
