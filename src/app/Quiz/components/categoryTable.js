...
const headCells = [
  ...
  { id: 'cat_img', numeric: true, disablePadding: false, label: 'Total Quiz', },
  { id: 'cat_img', numeric: true, disablePadding: false, label: 'Completed', },
  { id: 'cat_img', numeric: true, disablePadding: false, label: 'Latest', },
...
const fetchAllcategoriesAPI=useCallback(()=>{
  ...
  .then((response) => { getCategorylist(response.data.Data);  })
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="right"><Link href={`Quiz/QuizListing/${row._id}`}>{row.categoryName}</Link></TableCell>
                    <TableCell align="right">
                      <div style={{position:'relative',width:'50px',height:'50px',borderRadius:'50%',border:'1px solid #e2e2e2',marginLeft:'auto',overflow: 'hidden'}}><Image src={row.categoryImage} alt="product image" width={50} height={50}/>
                      </div></TableCell>
                    ...
                    <TableCell align="right"><Link href={`Quiz/${row._id}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
