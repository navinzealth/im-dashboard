...
const headCells = [
  ...
  { id: 'cat_name',  numeric: false, disablePadding: false, label: 'height', },
  { id: 'cat_district',  numeric: false, disablePadding: false, label: 'width', },
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" style={{'fontSize':'22px','fontWeight:':'600'}}>  Tree List</Typography>
      )}
    </Toolbar>
  );
}
...
const fetchAllProjects=useCallback(()=>{
  ...
  let config = {
    ...
    url: `${apiRoute}/completedorderdetail`,
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="left">{row.treeId}</TableCell>
                    <TableCell align="left">
                      <div style={{position:'relative',width:'50px',height:'50px',borderRadius:'50%',border:'1px solid #e2e2e2',overflow: 'hidden'}}><Image src={row.image} alt="product image" width={50} height={50}/>
                      </div></TableCell>
                    ...
                    <TableCell align="left"><Link href={`FulfilledOrders/${props.orderURL}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
