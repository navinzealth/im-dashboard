...
const headCells = [
  ...
  { id: 'cat_published',  numeric: true, disablePadding: false, label: 'Published', },    { id: 'cat_date', numeric: true, disablePadding: false, label: 'Date', },
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" >  Blogs </Typography>
      )}
    </Toolbar>
  );
}
 ...
 // const visibleRows =  useMemo( () =>  rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage,),  [ page, rowsPerPage], );
  ...
  const changeBlogStatus = (event) => { 
   ...
   formdata.append("status", event.status=='1' ? '0' : '1');
    ...
    fetch(`${apiRoute}/editblog`, requestOptions)
      .then(response => response.text())
     // .then(result => console.log(result))
      .then(fetchAllBlogsAPI) // fetch again all blog api after sending post request of changinh status
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="right">
                      <div style={{position:'relative',width:'50px',height:'50px',borderRadius:'50%',border:'1px solid #e2e2e2',marginLeft:'auto',overflow: 'hidden'}}><Image src={row.blog_image} alt="product image" width={50} height={50}/>
                      </div></TableCell>
                    
                    <TableCell align="right">{row.blog_title}</TableCell>
                    <TableCell align="right">{row.short_desc}</TableCell>
                    
                    <TableCell align="right"><Switch checked={ JSON.parse(row.status) == 1 ? true : false} 
                    inputProps={{ 'aria-label': 'controlled' }} onChange={()=>changeBlogStatus(row)}/></TableCell>										<TableCell align="right">{`${new Date(row.createdAt).getDate()}/${new Date(row.createdAt).getMonth()+1}/${new Date(row.createdAt).getFullYear()}`}</TableCell>
                    <TableCell align="right"><Link href={`Blogs/${row.blogId}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
