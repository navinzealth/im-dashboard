...
const headCells = [
  ...
  { id: 'cat_district',  numeric: false, disablePadding: false, label: 'District', },
  { id: 'cat_state',  numeric: false, disablePadding: false, label: 'State', },
  { id: 'cat_pincode',  numeric: false, disablePadding: false, label: 'Pincode', },
];
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" style={{'fontSize':'22px','fontWeight:':'600'}}>  Projects List</Typography>
      )}
    </Toolbar>
  );
}
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="left"><Link href={`projects/${row.projectId}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
